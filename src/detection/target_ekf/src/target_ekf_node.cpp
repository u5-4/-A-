...
void update_state_callback(const object_detection_msgs::BoundingBoxesConstPtr &bboxes_msg, const nav_msgs::OdometryConstPtr &odom_msg) {
  ...
  std::cout << "odom pose: " << odom_p.transpose() << std::endl;
  std::cout << "cam pose : " << cam_p.transpose() << std::endl;
  ...
  //double x = ((xmin + xmax) * 0.5 - cx_) * depth / fx_;
	...
	tf2::Quaternion quat;
	tf2::convert(odom_msg->pose.pose.orientation, quat); //把mavros/local_position/pose里的四元数转给tf2::Quaternion quat
	double now_roll, now_pitch, now_yaw;
	tf2::Matrix3x3(quat).getRPY(now_roll, now_pitch, now_yaw);

  //std::cout << "now yaw: " << now_yaw*RtoD << std::endl;
  //std::cout << "now pitch: " << now_pitch*RtoD << std::endl;
  //std::cout << "now_roll: " << now_roll * RtoD<< std::endl;

  double yaw_target  =  std::fabs(std::atan2((xmin + xmax) * 0.5 - cx_,fx_));
  double roll_target= std::fabs(now_roll);
  double pitch_target= std::fabs(std::atan2(ymax - cy_,fy_)+now_pitch);
  double depth =cam_p.z();
  //double depth =cam_p.z()/(std::cos(pitch_target )*std::cos(roll_target)); 
  //depth=depth /std::cos(yaw_target);
  double y = ((ymin + ymax) * 0.5 - cy_) * depth / fy_;
  double x = ((xmin + xmax) * 0.5 - cx_) * depth / fx_;
  //std::cout << "x  : " << x<<"  y  : "<< y << std::endl;
  Eigen::Vector3d p(-y+cam2body_p_.x(), -x, 0);
  
  Eigen::Matrix3d wrbr0 = uav_utils::rotz(now_yaw);
  p = wrbr0 * p +odom_p;
  ...
  std::cout << "error       : " << (p-odom_p).transpose() << std::endl;
  // publish yolo odom
